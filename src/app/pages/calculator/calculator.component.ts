...
export class CalculatorComponent {
  ...
  constructor(private formBuilder: FormBuilder, private clipboard: Clipboard, private visitService: VisitService ) {
  ...
  oblicz() {
    if (this.formGroup.valid) {
      ...
      const powodzwrot = (kosztyPowod * (procentWygranejPowoda / 100)).toFixed(2);
      const pozwanyzwrot = (kosztyPozwany * (procentPrzegranejPowoda / 100)).toFixed(2);
      ...
      Kompensując ww. kwoty ${przegrany} ma zwrócić ${wygrany} ${
        ostatecznaKwota.toFixed(2).toLocaleString()
      } złotych.
      ...
      this.wynik += `<br>${przegrany} wygrał w ${procentPrzegranejPowoda.toFixed(2)}%.`;
  
      if (ostatecznaKwota > 0) {
        this.wynik += `<br>${wygrany} wygrał(a) i otrzymuje ${ostatecznaKwota.toFixed(2)} zł.`;
